	...
	$.ajax({
		...
		success:function(response)
		{
			...
			if(response)
			{
				...
				for(var reg in response)
				{
					html += '<p class="menu-titulo">'+response[reg].descripcion+'</p>'
					html += '<div class="col-12" id="sploo'+response[reg].id+'"></div>';
	...
	$.ajax({
		...
		success:function(response)
		{
			...
			for (var reg in response){
				...
				if(response[reg].foto != "" || null)
				{
				    html += '<img src="'+response[reg].foto+'" style="width:100px"/>';    
				}else {
                    html += '<img src="https://www.publicdomainpictures.net/pictures/280000/velka/not-found-image-15383864787lu.jpg" style="width:100px"/>'; 
                }
